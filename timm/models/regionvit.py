...
from timm.layers import AttentionWithRelPosViT
from timm.layers import LayerNorm2dViT
        ...
        if patch_conv_type == '3conv':
            if patch_size[0] == 4:
                tmp = [
                    ...
                    LayerNorm2dViT(embed_dim // 4),
                    nn.GELU(),
                    nn.Conv2d(embed_dim // 4, embed_dim // 2, kernel_size=3, stride=2, padding=1),
                    LayerNorm2dViT(embed_dim // 2),
                    nn.GELU(),
...
class R2LAttentionPlusFFN(nn.Module):
    ...
    def __init__(self, input_channels, output_channels, kernel_size, num_heads, mlp_ratio=1., qkv_bias=False,
                 qk_scale=None,
        ...
        self.attn = AttentionWithRelPosViT(
            input_channels, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop,
            proj_drop=drop,
        ...
        self.mlp = Mlp(in_features=input_channels, hidden_features=int(output_channels * mlp_ratio),
                       out_features=output_channels, act_layer=act_layer, drop=drop)
...
def convert_to_flatten_layout(cls_tokens, patch_tokens, ws):
    ...
    patch_tokens = tmp.transpose(1, 2).reshape(-1, C, kernel_size[0] * kernel_size[1]).transpose(-2,
                                                                                                 -1)  # (NxH/sxK/s)x(ksxks)xC
...
class ConvAttBlock(nn.Module):
    def __init__(self, input_channels, output_channels, kernel_size, num_blocks, num_heads, mlp_ratio=1.,
                 qkv_bias=False, qk_scale=None, pool='sc',
        ...
        for i in range(num_blocks):
            ...
            tmp.append(
                R2LAttentionPlusFFN(output_channels, output_channels, kernel_size_, num_heads, mlp_ratio, qkv_bias,
                                    qk_scale,
                                    act_layer=act_layer, norm_layer=norm_layer, drop_path=drop_path_rate[i],
        ...
        self.peg = nn.Conv2d(output_channels, output_channels, kernel_size=3, padding=1, groups=output_channels,
                             bias=False) if peg else None
    ...
    def forward(self, xs):
        ...
        for i in range(1, len(self.block)):
            ...
            if self.peg is not None and i == 1:
                cls_tokens, patch_tokens = convert_to_spatial_layout(out, self.output_channels, B, H, W,
                                                                     self.kernel_size, mask, p_l, p_r, p_t, p_b)
                cls_tokens = cls_tokens + self.peg(cls_tokens)
                patch_tokens = patch_tokens + self.peg(patch_tokens)
                out, mask, p_l, p_r, p_t, p_b, B, C, H, W = convert_to_flatten_layout(cls_tokens, patch_tokens, self.ws)

        cls_tokens, patch_tokens = convert_to_spatial_layout(out, self.output_channels, B, H, W, self.kernel_size, mask,
                                                             p_l, p_r, p_t, p_b)
        return cls_tokens, patch_tokens
...
class RegionViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=(768,), depth=(12,),
        ...
        if self.det_norm:
            ...
            for i in range(4):
                layer = LayerNorm2dViT(embed_dim[1 + i])
                layer_name = f'norm{i}'
...
@torch.no_grad()
def _load_weights(model: RegionViT, checkpoint_path: str, prefix: str = ''):
    """Load pretrianed weights to model.

    Args:
        model (RegionViT): model to load weights
        checkpoint_path (str): path to pretrained weights
        prefix (str, optional): prefix of pretrained weights. Defaults to ''.
    """
    import numpy as np

    if checkpoint_path.endswith('.npz'):
        # original RegionViT checkpoint
...
default_cfgs = {
    "regionvit_tiny_w14_224": _cfg(
        url='https://storage.googleapis.com/vit_models/augreg/Ti_16-i21k-300ep-lr_0.001-aug_none-wd_0.03-do_0.0-sd_0.0--imagenet2012-steps_20k-lr_0.03-res_224.npz',
        hf_hub_id=None,
        custom_load=True
    ),
    "regionvit_base_224": _cfg(
        url="https://github.com/IBM/RegionViT/releases/download/weights-v0.1/RegionViT-B.pth",
        hf_hub_id=None,
        custom_load=False
    ),
}
...
def _create_region_vit(variant: str, pretrained: bool = False, **kwargs) -> RegionViT:
    return build_model_with_cfg(RegionViT, variant, pretrained, **kwargs)


@register_model
def regionvit_tiny_w14_224(pretrained=False, **kwargs):
    model_cfg = _model_cfg['tiny_w14']
    model = _create_region_vit('regionvit_tiny_w14_224', pretrained=pretrained, **dict(model_cfg, **kwargs))
...
def regionvit_tiny_224(pretrained=False, **kwargs):
    ...
    model = _create_region_vit('regionvit_tiny_224', pretrained=pretrained, **dict(model_cfg, **kwargs))
...
def regionvit_small_224(pretrained=False, **kwargs):
    ...
    model = _create_region_vit('regionvit_small_224', pretrained=pretrained, **dict(model_cfg, **kwargs))
...
def regionvit_small_w14_224(pretrained=False, **kwargs):
    ...
    model = _create_region_vit('regionvit_small_w14_224', pretrained=pretrained, **dict(model_cfg, **kwargs))
...
def regionvit_small_w14_peg_224(pretrained=False, **kwargs):
    ...
    model = _create_region_vit('regionvit_small_w14_peg_224', pretrained=pretrained, **dict(model_cfg, **kwargs))
...
def regionvit_medium_224(pretrained=False, **kwargs):
    ...
    model = _create_region_vit('regionvit_medium_224', pretrained=pretrained, **dict(model_cfg, **kwargs))
...
def regionvit_base_224(pretrained=False, **kwargs):
    ...
    model = _create_region_vit('regionvit_base_224', pretrained=pretrained, **dict(model_cfg, **kwargs))
...
def regionvit_base_w14_224(pretrained=False, **kwargs):
    ...
    model = _create_region_vit('regionvit_base_w14_224', pretrained=pretrained, **dict(model_cfg, **kwargs))
...
def regionvit_base_w14_peg_224(pretrained=False, **kwargs):
    ...
    model = _create_region_vit('regionvit_base_w14_peg_224', pretrained=pretrained, **dict(model_cfg, **kwargs))
