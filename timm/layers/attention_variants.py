...
class AttentionWithRelPosViT(nn.Module):
    ...
    def forward(self, x, patch_attn=False, mask=None):
        ...
        if self.rel_pos is not None and patch_attn:
            ...
            rel_pos = self.rel_pos[:, self.rel_pos_index.to(attn.device)].reshape(self.num_heads,
                                                                                  N - self.num_cls_tokens,
                                                                                  N - self.num_cls_tokens)
            attn[:, :, self.num_cls_tokens:, self.num_cls_tokens:] = attn[:, :, self.num_cls_tokens:,
                                                                     self.num_cls_tokens:] + rel_pos
