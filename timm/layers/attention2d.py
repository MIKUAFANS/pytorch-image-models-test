...
class LayerNorm1dViT(nn.Module):
    def __init__(self, channels, eps: float = 1e-5, elementwise_affine: bool = True) -> None:
        super(LayerNorm1dViT, self).__init__()
...
class Attention2dViT(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = qkv.reshape(B, 3, self.num_heads, self.out_dim // self.num_heads, H * W).permute(1, 0, 2, 4,
                                                                                               3)  # B(3C)N -> B3H(C/H)N -> 3BHN(C/H)
        q, k, v = qkv[0], qkv[1], qkv[2]  # make torch script happy (cannot use tensor as tuple), each BHN(C/H)
